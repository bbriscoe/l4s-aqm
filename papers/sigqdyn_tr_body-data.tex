...
\begin{figure}[h]
	\centering
	\includegraphics[width=\columnwidth]{marking-fairness2}
	\caption{TBA}\label{fig:marking-fairness2}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=\columnwidth]{fq-marking-fairness}
	\caption{TBA}\label{fig:fq-marking-fairness}
\end{figure}

\begin{figure}[h]
	\centering
	\includegraphics[width=\columnwidth]{fq-marking-fairness2}
...
TCP Segmentation Offload (TSO) in Linux (and possibly other OSs) groups a set of packet into a back-to-back burst and calculates the maximum packets in the burst from the current pacing rate of the flow:
\begin{verbatim}
...
If an AQM at the bottleneck is based on sojourn time and therefore marks larger bursts less than smaller bursts, new flows will attract more of the congestion marking. Then as a new flow tries to displace an established flow, it will tend to reach a point of local equilibrium before it has reached the same rate as the established flow. \todo{ToDo: Got to here - add figures from JM's paper}.
